...
class Auth extends CI_Controller
{
	...
	function logout()
	{
		...
		/* if(!$this -> dbm_model -> backup_database())
		...
		if($data['user_type'] == 'superadmin' || $data['user_type'] == 'admin' || $data['user_type'] == 'manager' )
		{
			...
			else 
			{
				...
				foreach($query -> result() as $field):
					$temp[ $field -> hotel_id ] = $field -> hotel_name.' ( '.$field -> hotel_address .' )';
				endforeach;
				$data['all_shop'] = $temp;
...
echo $login;
	...
	function access_auth()
	{
		...
		$data['bd_date'] = date('Y-m-d',time());
		$this->load->model('access_control_model');
		
		$segment_3 = $this->uri->segment(3);
		if($segment_3){
			$data['access_info'] = $this->access_control_model->get_user_authentication($segment_3);
		}
		
		$this->load->view('access_auth_view',$data);
	}
	
	function create_access_authentication(){
		$module = $this->input->post('module');
		$myvalue = $this->input->post('myvalue');

		if($this->tank_auth->get_usertype() == 'superadmin'){
			foreach($myvalue as $myvalue){

				if(isset($module[$myvalue])){
					$udata = array('value'=>1);
					$this->db->where('access_auth_id',$myvalue);
					$this->db->update('rest_access_auth',$udata);
				}
				else{
					$udata = array('value'=>0);
					$this->db->where('access_auth_id',$myvalue);
		...
		redirect('auth/access_auth/'.$this->input->post('user_type').'/success');
	}
